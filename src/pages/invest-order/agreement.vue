<script setup lang="ts" name="InvestAgreement">
    ...
    <div class="agreement" v-else>
      ...
      <div class="box-base">
        <van-grid column-num="2" :center="false">
          ...
          <van-grid-item style="flex-basis: 65%" :text="info.item_name" />
          ...
          <van-grid-item style="flex-basis: 65%" :text="`${info.amount} ${currency}`" />
      ...
      <div class="box-2">
        <div>
          <div>甲方（管理方）：Iberdrola（伊比德罗拉）能源投资（中国）有限公司</div>
          <div style="text-align: end">{{ dayjs(info.create_at).format('YYYY-MM-DD') }}</div>
        </div>
        <div>
          <div>乙方（投资方）：{{ userInfo.sfz_name }} ({{ userInfo.sfz_number }})</div>
          <div style="text-align: end">{{ dayjs(info.create_at).format('YYYY-MM-DD') }}</div>
