  ...
  function JSBridge() {
    ...
    function _init(config) {
      ...
      if (config) {
        ...
        if (
          config.channel &&
          Object.prototype.toString.apply(config.channel) === '[object Array]'
        ) {
      ...
      _registerHandler('#evalJavaScript#', function (data, success, fail) {
        try {
          success(
            ...
            Function('"use strict";return (' + data.toString() + ')')()
          )
        } catch (error) {
          _log(error)
          fail(error.toSring())
      ...
      if (_isFlutterInAppWebView) {
        if (window.flutter_inappwebview) {
          if (window.flutter_inappwebview.callHandler) {
            window.flutter_inappwebview.callHandler('FlutterWebView', encodeString)
          } else if (window.flutter_inappwebview._callHandler) {
            window.flutter_inappwebview._callHandler(
              'FlutterWebView',
              setTimeout(function () {}),
              JSON.stringify([encodeString])
            )
          } else {
